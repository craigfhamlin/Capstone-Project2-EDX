...
trains2 %>%
  ...
  labs(
    ...
    x = "Rank", y = "PPP", fill = element_blank()
  ) +
  theme_classic()
...
ptm <- proc.time()
xgbTree_step22 <- train(rank ~ .,
                        data = trains2,
                        method = "xgbTree",
                        trControl = fit_Control,
                        tuneGrid = tune_grid)
tm <- proc.time() - ptm
grid44 <- accuracy(xgbTree_step22, 'xgbTree - Tune1', trains2, tests2, tm[[3]])
...
tune_grid <- expand.grid(
  nrounds = seq(from = 100, to = 1000, by = 50),
  eta = xgbTree_step22$bestTune$eta,  
  max_depth = c(2, 3, 4, 5),
  gamma = 0,
  colsample_bytree = 1,
  min_child_weight = 1,
...
ptm <- proc.time()
xgbTree_step12 <- train(rank ~ .,
                        data = trains2,
                        method = "xgbTree",
                        trControl = fit_Control,
                        tuneGrid = tune_grid)
tm <- proc.time() - ptm
grid4 <- accuracy(xgbTree_step12, 'xgbTree - Tune2', trains2, tests2, tm[[3]])
...
tune_grid <- expand.grid(
  nrounds = seq(from = 100, to = 1000, by = 50),
  eta = xgbTree_step22$bestTune$eta,
  max_depth = xgbTree_step12$bestTune$max_depth,
  gamma = 0,
  colsample_bytree = 1,
...
xgbTree_step22 <- train(rank ~ .,
                        ...
                        tuneGrid = tune_grid)
tm <- proc.time() - ptm
grid5 <- accuracy(xgbTree_step22, 'xgbTree - Tune3', trains2, tests2, tm[[3]])
...
tune_grid <- expand.grid(
  ...
  min_child_weight = xgbTree_step22$bestTune$min_child_weight,
  subsample = c(0.5, 0.75, 1.0)
)


ptm <- proc.time()
xgbTree_step32 <- train(rank ~ .,
                        data = trains2,
                        method = "xgbTree",
                        trControl = fit_Control,
                        tuneGrid = tune_grid)
tm <- proc.time() - ptm
grid6 <- accuracy(xgbTree_step32, 'xgbTree - Tune4', trains2, tests2, tm[[3]])
...
## ---- gamma tuned xgbTree-------------------------------------------------------
tune_grid42 <- expand.grid(
  nrounds = seq(from = 100, to = 1000, by = 50),
  eta = xgbTree_step22$bestTune$eta,
  max_depth = xgbTree_step12$bestTune$max_depth,
  gamma = c(0, 0.05, 0.1, 0.4, 0.6, 0.8, 1.0),
  colsample_bytree = xgbTree_step32$bestTune$colsample_bytree,
  min_child_weight = xgbTree_step22$bestTune$min_child_weight,
  subsample = xgbTree_step32$bestTune$subsample
)

ptm <- proc.time()
xgbTree_step42 <- train(rank ~ .,
                        data = trains2,
                        method = "xgbTree",
                        trControl = fit_Control,
                        tuneGrid = tune_grid)
tm <- proc.time() - ptm
grid7 <- accuracy(xgbTree_step42, 'xgbTree - Tune5', trains2, tests2, tm[[3]])
...
rbind(grid,grid2,grid3,grid44,grid4,grid5,grid6,grid7) %>%
kable(booktabs = T)  


## ---- plot variable importance--------------------------------------------------
importance <- varImp(xgbTree_step42)
plot(importance)
