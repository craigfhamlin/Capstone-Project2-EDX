...
In this project a publicly available dataset was employed to create machine learning models which predicted the rank of a National Hockey Leagure players team.
...
The above graph displays ppg by age regression lines with confidence bands for each rank of team.
...
Using a system of guessing we obtained an accuracy of `r round(mean(guess == trains2$rank),3)` for the training data and `r round(mean(guess == tests2$rank), 3)` for the test data. 
...
We see a definite increase in the accuracy of the model once the initial hyperparameter tuning was performed. There is a substantial increase in accuracy for both the training and test sets. The best tune for learning rate was `r xgbTree_step22$bestTune$eta`.
...
In this model training accuracy has increased slightly however the test set accuracy is slightly reduced. The best tune for maximum tree depth was `r xgbTree_step12$bestTune$max_depth`. 

#### Tuning minimum child weight

For the next step in tuning, the minimum child depth was expanded to include a range of values centered around the default value.
...
With this tuning variation we see a slight change in results. Both he training and test accuracy were improved upon. The best tune for minimum child weight in this model was `r xgbTree_step22$bestTune$min_child_weight`
...
After tuning these hyperparameters there was a large jump in the accuracy of the model. Both the training and test accuracy received significant improvement.  The best tune for column subsample in this model was `r xgbTree_step32$bestTune$colsample_bytree` and the best tune for subsample ratio of training observations was `r xgbTree_step32$bestTune$subsample`


#### Tuning the Gamma hyperparameter

Gamma is a regularization parameter for xgbTree which represents how much the loss will be reduced by for the model to perform a split resulting in a tree.The higher the level of Gamma the higher the degree of regularization in the model. In this step the model was performed with a degree of Gamma sequenced from 0 to 1 by irregular intervals. 
...
By altering the Gamma values the best tuned model once again had noticable increase in both training and test accuracy. 
...
The final tuning of hyperparameters produced the best results for both the training and test set accuracy. The final model has surpassed the training set classifier for baseline accuracy by `r round(grid7$Accuracy..train. - train_guess,3)`
...
Four top variables really stand out for importance (X..P, CAP.HIT, TOI.GP, PPG), while four variables are middling (PPPG,  GP, AGE, P) and two variables are low importance (PosD, PosL). PosR is also listed but the value is essentially negligible.
